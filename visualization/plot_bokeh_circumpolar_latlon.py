...
Usage: python plot_bokeh_circumpolar_latlon.py <file_path> [variable_name]
...
def create_bokeh_app(file_path, variable_name=None):
...
- NEE calculation (Net Ecosystem Exchange = RECO - GPP)
    ...
    # Require file_path if not using --nee
    if args.file_path is None:
        parser.error("file_path is required when not using --nee flag")
    ...
    make_doc = create_bokeh_app(args.file_path, args.variable)
