...
def pull_single_batch_restart_file(bucket_path, tile_name, scenario_name, batch_name):
    """
    Pulls a single restart-sc.nc file from one batch in GCS.
    
    Args:
        bucket_path: GCS bucket path (e.g., "circumpolar_model_output/recent2")
        tile_name: Tile name (e.g., "H10_V10")
        scenario_name: Scenario name (e.g., "ssp1_2_6_mri_esm2_0_split")
        batch_name: Batch folder name (e.g., "batch_0")
    
    Returns:
        True if successful, False if failed
    """
    # Construct paths
    gcs_file_path = f"gs://{bucket_path}/{tile_name}/{scenario_name}/{batch_name}/output/restart-sc.nc"
    local_output_dir = f"{tile_name}_sc/{scenario_name}/{batch_name}/output"
    local_file_path = f"{local_output_dir}/restart-sc.nc"
    
    # Create local directory structure
    os.makedirs(local_output_dir, exist_ok=True)
    
    try:
        print(f"[PULL] Pulling {batch_name}/output/restart-sc.nc ...")
        run_cmd(f"gsutil cp {gcs_file_path} {local_file_path}")
        print(f"[PULL] Successfully downloaded restart-sc.nc for {batch_name}")
        return True
    except Exception as e:
        if "No URLs matched" in str(e) or "BucketNotFoundException" in str(e):
            print(f"[WARNING] File not found: {gcs_file_path}")
        else:
            print(f"[WARNING] Failed to download restart-sc.nc for {batch_name}")
        print(f"[WARNING] Details: {e}")
        return False


def pull_restart_files_for_scenario(bucket_path, tile_name, scenario_name):
    """
    Pulls restart-sc.nc files from all batches of a scenario in GCS.
    Discovers batches from the local filesystem structure.
    
    Args:
        bucket_path: GCS bucket path (e.g., "circumpolar_model_output/recent2")
        tile_name: Tile name (e.g., "H10_V10")
        scenario_name: Scenario name (e.g., "ssp1_2_6_mri_esm2_0")
    """
    # Append _split to scenario name for the local directory
    scenario_name_split = scenario_name if scenario_name.endswith("_split") else f"{scenario_name}_split"
    
    # Discover batches from local filesystem
    local_scenario_path = f"{tile_name}_sc/{scenario_name_split}"
    
    if not os.path.exists(local_scenario_path):
        print(f"[ERROR] Local scenario directory not found: {local_scenario_path}")
        print(f"[ERROR] Cannot discover batches. Please ensure the scenario directory exists.")
        return
    
    # Find all batch_* directories
    batch_dirs = [
        name for name in os.listdir(local_scenario_path)
        if os.path.isdir(os.path.join(local_scenario_path, name)) and name.startswith("batch_")
    ]
    
    if not batch_dirs:
        print(f"[WARNING] No batch directories found in {local_scenario_path}")
        return
    
    # Sort batch directories for consistent ordering
    batch_dirs.sort(key=lambda x: int(x.split("_")[1]) if x.split("_")[1].isdigit() else 0)
    
    print(f"[PULL] Found {len(batch_dirs)} batches in {local_scenario_path}")
    print(f"[PULL] Batches to process: {', '.join(batch_dirs)}")
    
    # Pull restart-sc.nc for each batch
    successful = 0
    failed = 0
    
    for batch_name in batch_dirs:
        success = pull_single_batch_restart_file(bucket_path, tile_name, scenario_name_split, batch_name)
        if success:
            successful += 1
        else:
            failed += 1
    ...
    # Print summary
    print(f"\n[PULL] Summary for {tile_name}/{scenario_name_split}:")
    print(f"[PULL]   Successfully pulled: {successful}/{len(batch_dirs)} restart-sc.nc files")
    if failed > 0:
        print(f"[PULL]   Failed to pull: {failed}/{len(batch_dirs)} restart-sc.nc files")
    print(f"[PULL] Restart file pull completed.")
...
def conform_runmask(tile_name):
    ...
    env = os.environ.copy()
    dvm_scripts_path = "/home/dteber_woodwellclimate_org/fire/dvm-dos-tem/scripts"
    ...
    params_dir = "/home/dteber_woodwellclimate_org/fire/dvm-dos-tem/parameters/"
    runmask_script = "/home/dteber_woodwellclimate_org/fire/dvm-dos-tem/scripts/util/runmask.py"
...
def split_base_scenario(path_to_folder, tile_name, base_scenario_name):
    ...
    run_cmd(f"bp batch split -i {input_path} -b {output_path} --restart-run --p 0 --e 0 --s 0 --t 124 --n 0")
    return output_path
...
def main():
    ...
    if args.mode == "full":
        ...
        # full pipeline uses your splitter to derive the base scenario path
        # base_split_path = split_base_scenario(path_to_folder, tile_name, base_scenario_name)
        pull_restart_files_for_scenario("circumpolar_model_output/recent2", tile_name, base_scenario_name)
        # run_batch_scenario(base_split_path)
        # wait_for_jobs()
        # resubmit_unfinished_jobs(base_split_path)
        # wait_for_jobs()
