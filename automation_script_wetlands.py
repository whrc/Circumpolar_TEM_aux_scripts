...
def split_base_scenario(path_to_folder, tile_name, base_scenario_name):
    ...
    run_cmd(f"bp batch split -i {input_path} -b {output_path} --p 100 --e 1000 --s 250 --t 124 --n 76")
    return output_path
...
def finalize(path_to_folder, tile_name):
    ...
    destination_path = f"gs://circumpolar_model_output/wetland/{tile_name}"
...
def main():
    ...
    if args.mode == "full":
        ...
        # mask non-wetlands
        run_cmd(f"python ~/Circumpolar_TEM_aux_scripts/mask_by_veg.py --run-mask {path_to_folder}/input_tiles/{tile_name}/run-mask.nc --vegetation {path_to_folder}/input_tiles/{tile_name}/vegetation.nc --output {path_to_folder}/input_tiles/{tile_name}/run-mask-wetland.nc --veg-values 6 31 32 55 61 75 92 93")
        run_cmd(f"mv {path_to_folder}/input_tiles/{tile_name}/run-mask.nc {path_to_folder}/input_tiles/{tile_name}/run-mask-original.nc")
        run_cmd(f"mv {path_to_folder}/input_tiles/{tile_name}/run-mask-wetland.nc {path_to_folder}/input_tiles/{tile_name}/run-mask.nc")

        generate_scenarios(tile_name)
        conform_runmask(tile_name, base_scenario_name)
        remove_tile(tile_name)
        shutil.copyfile('/home/ext_bmaglio_alaska_edu/ch4_inputs/ch4.nc', path_to_folder+'/'+tile_name+'_sc/'+'ssp1_2_6_mri_esm2_0/ch4.nc')
        shutil.copyfile('/home/ext_bmaglio_alaska_edu/ch4_inputs/projected-ch4_ssp1_2_6.nc', path_to_folder+'/'+tile_name+'_sc/'+'ssp1_2_6_mri_esm2_0/projected-ch4.nc')
        shutil.copyfile('/home/ext_bmaglio_alaska_edu/ch4_inputs/ch4.nc', path_to_folder+'/'+tile_name+'_sc/'+'ssp5_8_5_mri_esm2_0/ch4.nc')
        shutil.copyfile('/home/ext_bmaglio_alaska_edu/ch4_inputs/projected-ch4_ssp5_8_5.nc', path_to_folder+'/'+tile_name+'_sc/'+'ssp5_8_5_mri_esm2_0/projected-ch4.nc')
